...
public:
	...
	Node<Key,Value>* helper(Node<Key,Value>* parent,Node<Key,Value>*node, const std::pair<Key, Value>& keyValuePair);
	Node<Key,Value>* helper1(Node <Key,Value> *node,const Key& key) const;
	void helper2(Node<Key,Value>*node,Node<Key,Value>*parent);
	void clear();
	void print() const;
...
{
	// TODO
	mRoot = helper(NULL,mRoot,keyValuePair);
}


template<typename Key, typename Value>
Node<Key,Value>* BinarySearchTree<Key, Value>::helper(Node<Key,Value> *parent,Node<Key,Value> *node, const std::pair<Key, Value>& keyValuePair){
if(node == NULL){
	Node <Key,Value> * item = new Node <Key,Value> (keyValuePair.first,keyValuePair.second,parent);
	node=item;
}
...
template<typename Key, typename Value>
void BinarySearchTree<Key, Value>::helper2(Node<Key,Value>*node,Node<Key,Value>*parent){
	if(node->getLeft()==NULL and node->getRight() == NULL ){
		if(node->getParent()!=NULL){
		 ...
		 helper2(node->getLeft(),node);
	}
	else if(node->getRight()!=NULL){
		helper2(node->getRight(),node);
	}
}
...
Node<Key, Value>* BinarySearchTree<Key, Value>::internalFind(const Key& key) const
{
	...
	return helper1(mRoot,key);
}

template<typename Key, typename Value>
Node<Key,Value>* BinarySearchTree<Key, Value>::helper1(Node<Key,Value>*node,const Key& key) const{
    ...
    // go left
    if(key <= node->getKey()){
        return helper1(node->getLeft(), key);
    }
    // go right
    if(key>mRoot->getKey()){
        return helper1(node->getRight(), key);
    }
}
